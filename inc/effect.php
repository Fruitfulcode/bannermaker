...
function get_Banner_slide($atts)	{
	...
	if($animation_ef == 'Fade') {
		$out['slides'] = '<div class="banner_slide" id="slide-layer-'.$count.'" style="display: none;left: '.$style_x.';top: '.$style_y.';z-index: '.$count.';display:block;">'.$temp_body.'</div>';
		$out['reset']  = 'jQuery("#slide-layer-'.$count.'").css({display: "none",left: "'.$style_x.'",top: "'.$style_y.'"});';
	} else {
		$out['slides'] = '<div class="banner_slide" id="slide-layer-'.$count.'" style="left: '.$style_x.';top: '.$style_y.';z-index: '.$count.';display:block;">'.$temp_body.'</div>';
		$out['reset']  = 'jQuery182("#slide-layer-'.$count.'").css({left: "'.$style_x.'",top: "'.$style_y.'"});';
	}
	/* Animation */
	
	if( $animation_ef == 'Fade')	{	$out['scripts'] = 'jQuery("#slide-layer-'.$count.'").delay('.$delay.').fadeIn('.$speed.')'; $prefix = ';jQuery182("#slide-layer-'.$count.'")'; }
	...
	if(	$animation_ef == 'Short from Top' 	 or 
		...
		$animation_ef == 'Short from Left'	)
									{	$out['scripts'] = 'jQuery182("#slide-layer-'.$count.'").delay('.$delay.').animate({left: "'.$val_x.'", top: "'.$val_y.'"},'.$speed.',"'.$easing.'")'; }
	...
	if(	$animation_ef == 'Long from Top' 	 or 
		...
		$animation_ef == 'Long from Left'	)
									{	$out['scripts'] = 'jQuery182("#slide-layer-'.$count.'").delay('.$delay.').animate({left: "'.$val_x.'", top: "'.$val_y.'"},'.($speed*2).',"'.$easing.'")'; }
